...
class PreferenceStore {
  ...
  func saveTargets(_ targets: [String: String]) {
    let jsonArray = targets.map { (uuid, name) -> [String: String] in
      return ["mac": uuid, "name": name]
    }
  ...
  func getLastNotification(uuid: String) -> Int64 {
    let val = defaults.object(forKey: KEY_LAST_NOTIFY_PREFIX + uuid) as? Int64
    return val ?? 0
  }

  func setLastNotification(uuid: String, timestamp: Int64) {
    defaults.set(timestamp, forKey: KEY_LAST_NOTIFY_PREFIX + uuid)
  }
  
  func clear(uuid: String) {
    defaults.removeObject(forKey: KEY_LAST_NOTIFY_PREFIX + uuid)
  }
